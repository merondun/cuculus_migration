...
#file with the invariant sites, GOOD sites! 
invariant=/dss/dsslegfs01/pr53da/pr53da-dss-0021/projects/2023__MigratoryGenomics/analyses/msmc/unrelated_chyiyin_populationlevel/vcfs/
#genome-wide mappability mask
gwmask=/dss/dsslegfs01/pr53da/pr53da-dss-0021/projects/2023__MigratoryGenomics/analyses/msmc/unrelated_chyiyin_populationlevel/mappability_masks
#neutral sites
neutral=/dss/dsslegfs01/pr53da/pr53da-dss-0021/projects/2023__MigratoryGenomics/analyses/msmc/unrelated_chyiyin_populationlevel/neutral-sites_cuckoo__intergenic-intron-4fold.CHRstrip.bed
#this bed contains BAD sites which are repeats (bed does not include 'low_complexity' and 'simple_repeats', so it's largely TEs)
repeats=/dss/dsslegfs01/pr53da/pr53da-dss-0021/projects/2023__MigratoryGenomics/analyses/msmc/unrelated_chyiyin_populationlevel/GCA_017976375.1_bCucCan1.pri_genomic.AvianRepeatModeler_Repeats-ExcludingLowComplexity-SimpleRepeats.CHRstrip.bed
...
rm crosscoal/${P1}_${P2}_${IT}.${i}_POPvcf.list

for j in $(cat crosscoal/${P1}_${P2}_${IT}.inds); do

#No individual-level masks here, just population level invariant sites below 
echo "${msmcdir}/vcfs/${j}_chr_${i}.vcf.gz" >> crosscoal/${P1}_${P2}_${IT}.${i}_POPvcf.list

done

#Generate input
generate_multihetsep.py --chr ${i} --negative_mask $repeats --mask ${invariant}/chr_${i}.invariant.bed.gz --mask ${gwmask}/GCA_017976375.1_chr_${i}.mask.bed.gz --mask $neutral $(cat crosscoal/${P1}_${P2}_${IT}.${i}_POPvcf.list) > crosscoal/input/${P1}_${P2}_${IT}_${i}.multihetsep.txt

#Output some summary stats
getStats crosscoal/input/${P1}_${P2}_${IT}_${i}.multihetsep.txt | \
    awk -v c=${i} -v p1=${P1} -v p2=${P2} -v it=${IT} '{OFS="\t"}{print $0, c, p1, p2, it}' > ${msmcdir}/popgen/${P1}_${P2}_${IT}_${i}.popgen.txt

done

#Run MSMC2, first on each population, then on both together (exhaustive haplotype comparisons) simpler -p based on smaller genome than human 
